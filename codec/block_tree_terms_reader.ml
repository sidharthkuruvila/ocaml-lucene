...
let create { Segment_read_state.dir; segment_info; field_infos} =
  ...
  let index_block_size = Data_input.read_vint meta_input in
  if index_block_size != 128 then failwith "Unsupported block size";
  let num_fields = Data_input.read_vint meta_input in
  ...
  let rec loop n =
    if n = 0 then
     []
      ...
      let field = Data_input.read_vint meta_input in
      let num_terms = Data_input.read_vlong meta_input in
      if num_fields <= 0 then failwith "not enough terms";
      let root_code = Data_input.read_string meta_input in
      let field_info = Field_infos.get_field field_infos field in
      let sum_total_term_freq = Data_input.read_vlong meta_input in
      let sum_doc_freq = if field_info.index_options = Field_infos.Index_options.DOCS then sum_total_term_freq else Data_input.read_vlong meta_input in
      let doc_count = Data_input.read_vint meta_input in
      if version < 4 then failwith "No support for older indexes";
      let min_term = Data_input.read_string meta_input in
      let max_term = Data_input.read_string meta_input in
      ...
      let index_start_fp = Data_input.read_vlong meta_input in
      let index_input_clone = Data_input.copy index_input in
      Data_input.set_file_pointer index_input_clone (Int64.to_int index_start_fp);
(*    Printf.printf "index start fp = %d\n" (Int64.to_int index_start_fp);*)
  ...
  let index_length = Data_input.read_long meta_input in
  let terms_length = Data_input.read_long meta_input in
