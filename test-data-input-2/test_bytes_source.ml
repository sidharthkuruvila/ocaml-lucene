...
let test_read_bytes () =
  ...
  let expected = "This is a simple" in
  Alcotest.(check string) "Should read the string \"This is a simple\"" expected result;
...
let test_get_byte () =
  let src = M.of_bytes input in
  let result = M.get_byte src 15 in
  let expected = 'e' in
  Alcotest.(check char) "Should read the string \"This is a simple\"" expected result;
  let expected_position = 0 in
  let updated_position = M.get_position src in
