...
module Writer = Lz4_compression.Make_writer(Buffer_data_output)
...
let test_compress_using_lz4_compression () =
  let s = "catamaran and catamarans end" in
  let buffer = Buffer.create 0 in
  Writer.compress buffer s;
  let compressed = Buffer.contents buffer in
  Printf.printf "compressed: %s\n" compressed;
  let di = String_data_input.of_bytes compressed in
  let uncompressed = Reader.decompress di (String.length s) in
  Printf.printf "uncompressed: %s\n" uncompressed;
  Alcotest.(check string) "Compress an input string" s uncompressed
  
let tests = [
    "Decompress using lz4 compression", `Quick, test_decompress_using_lz4_compression;
    "Compress using lz4 compression", `Quick,  test_compress_using_lz4_compression;
]
